  ...
  return (
    <div className='flex flex-col gap-4px min-w-0 flex-1'>
      ...
      <div className='flex items-center justify-between gap-4px'>
        <span
          className='text-11px text-t-secondary truncate leading-none'
          title={title}
        >
          {title}
        </span>
        <span className='text-11px text-t-tertiary tabular-nums leading-none shrink-0'>
          {currentVal}
        </span>
      </div>
      ...
      >
        {barCount === 0 ? (
          ...
          /* Render bars using a <svg> foreignObject workaround isn't needed —
             we use percent widths via a wrapping foreignObject or just use flex.
             Since SVG percent-width rects work fine when the SVG has width=100%,
             we compute proportional positions in a viewBox. */
          bars.map((bar, i) => {
            ...
            const color =
              series.unit === 'pct' ? pctBarColor(bar.value) : countBarColor();
