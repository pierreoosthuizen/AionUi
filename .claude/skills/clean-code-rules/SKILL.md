...
description: >
  Apply Robert C. Martin's Clean Code conventions — naming, function size, comment hygiene, error handling — when refactoring readability. Trigger when reviewing or polishing code for clarity at the function/method level.
...
- When duplication, repeated switches, or primitive clusters appear, name the concept with an argument object, polymorphism, special case, or other small abstraction — **but only after the third real occurrence** (see Rule of 3 below).
- Before extracting a helper, interface, or wrapper, check three gates: (1) at least three real concrete occurrences, (2) the same concept and not just the same syntax, (3) the duplication is causing measurable change-cost pain. If any gate fails, leave the duplication.
...
## Anti-dogma rules (Six Myths)

- **Rule of 3 (Repeat Until Good)**: duplicate freely on the first and second occurrence. Only extract a shared abstraction when a third real, concrete instance arrives and the duplication is causing actual change-cost pain. Premature DRY produces wrappers, interfaces, and indirection without removing real cost.
- **Correctness before performance**: optimise only after measurement shows a real bottleneck in the actual workload. A working slow function beats a broken fast one. Reject benchmarks, micro-tweaks, and architectural scale-prep until production data justifies them.
- **Verify AI-generated code before trusting it**: when Claude (or any model) wrote the code, read it as if a stranger wrote it. Run the relevant test or execution path. Reject suggested helpers, wrappers, or interfaces that add layers without removing felt pain — AI bias is to over-engineer.
...
- If I extracted a new abstraction, was there a third real occurrence and concrete pain — or did I act on the second sighting?
- If AI generated any of this change, did I read and run it before trusting it?
